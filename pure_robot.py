...
RobotState.__str__ = lambda self: \
    f"RobotState(x={self.x}, y={self.y}, " \
    f"angle={self.angle}, state={"WATER" if self.state == WATER else
                                 "SOAP"  if self.state == SOAP else 
                                 "BRUSH"}"
...
def set_state(transfer, new_internal_state, state):
    if new_internal_state == "water":
        ...
        return state
    new_state = RobotState(state.x, state.y, state.angle, self_state)
